...
function MusicEvents({
    ...
    const [ musicEventsData, musicEventsLoading ] = useAllPrismicDocumentsByType('music_events')
	...
	return (
		<PageContentWrapper loading={loading}>
			<div className="software-resume">
                ...
                <h2>Past Events</h2>
                {pastEvents && pastEvents.sort(sortByDate).map(e => <OneEvent {...e.data} />)}
...
function OneEvent({address, date_and_time, description, link, organization, title}) {
