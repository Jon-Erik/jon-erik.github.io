...
function SoftwareResume({
  ...
  const [softwareResumeData, softwareResumeLoading] =
    useSinglePrismicDocument('software_resume')
  const main_header_html = asHTML(
    softwareResumeData && softwareResumeData.data.main_header
  )
  const education_html = asHTML(
    softwareResumeData && softwareResumeData.data.education
  )
  const work_experience_html = asHTML(
    softwareResumeData && softwareResumeData.data.work_experience
  )
  ...
  const loading =
    navbarDataLoading ||
    !softwareResumeLoading ||
    softwareResumeLoading.state !== 'loaded'
    ...
    <PageContentWrapper loading={loading}>
      <div className="software-resume">
        ...
        <div
          ...
          dangerouslySetInnerHTML={{ __html: work_experience_html }}
        />
        <div
          className="imported-html"
