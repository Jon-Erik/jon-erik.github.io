...
function Contact({ 
	...
	}).filter(v => v == false).length > 0 : false
	...
	async function submitForm(e) {
		...
		try {
			...
			grecaptcha.ready(function() {
				grecaptcha.execute(REACT_APP_RECAPTCHA_SITE_KEY, {action: 'submit'}).then(async function(captchaToken) {
	...
	return (
		<PageContentWrapper loading={loading} centerChildren={true}>
			<div className="contact">
				...
				<div className="links">
					{links.map((d) => (
						<ButtonLink key={d.route} route={d.route} text={`View ${d.title}`} />
					))}
				</div>
			</div>
		</PageContentWrapper>
